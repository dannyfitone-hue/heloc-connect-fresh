...
const products: Array<{
  ...
  { key: "payment_check", eyebrow: "MORTGAGE PAYMENT CHECK", title: "MORTGAGE PAYMENT CHECK", shortTitle: "AM I PAYING TOO MUCH?", short: "Compare your current mortgage payment with an estimated HELOC CONNECT option in seconds.", icon: "$", accent: "emerald" }
];
...
const goalsByProduct: Record<ProductKey, Array<{ title: string; desc: string }>> = {
  ...
  payment_check: [
    { title: "Check my current mortgage payment", desc: "See whether your current payment may be improved." }
  ]
};
...
function formatMoneyInput(v: string) { const clean = String(v || "").replace(/[^0-9]/g, ""); return clean ? Number(clean).toLocaleString("en-US") : ""; }
...
export default function LandingPage() {
  ...
  const [mortgageTermInput, setMortgageTermInput] = useState("30");
  ...
  const paymentCheckRemainingMonths = useMemo(() => {
    const totalMonths = Math.max(120, Number(mortgageTermInput || 30) * 12);
    if (!purchaseDateInput) return totalMonths;
    const purchased = new Date(`${purchaseDateInput}T00:00:00`);
    if (Number.isNaN(purchased.getTime())) return totalMonths;
    const now = new Date();
    const elapsed = Math.max(0, (now.getFullYear() - purchased.getFullYear()) * 12 + (now.getMonth() - purchased.getMonth()));
    return Math.max(12, totalMonths - elapsed);
  }, [mortgageTermInput, purchaseDateInput]);
  const paymentCheckCurrentPayment = useMemo(() => monthlyPayment(toNumber(mortgageBalanceInput), currentRate, paymentCheckRemainingMonths), [mortgageBalanceInput, currentRate, paymentCheckRemainingMonths]);
  const paymentCheckEstimatedPayment = useMemo(() => monthlyPayment(toNumber(mortgageBalanceInput), offerRate, TERM_MONTHS), [mortgageBalanceInput, offerRate]);
  const paymentCheckMonthlyDifference = Math.round(paymentCheckCurrentPayment - paymentCheckEstimatedPayment);
  const paymentCheckCanImprove = toNumber(mortgageBalanceInput) > 0 && paymentCheckCurrentPayment > 0 && paymentCheckMonthlyDifference >= 50;
  const paymentCheckComplete = Boolean(street.trim()) && Boolean(purchaseDateInput) && toNumber(mortgageBalanceInput) > 0 && toNumber(currentRateInput) > 0 && Number(mortgageTermInput) > 0;
  ...
  async function lookupPurchasePeriodValue(addressData: { address?: string; street?: string; city?: string; state?: string; zip?: string }, purchaseDate = purchaseDateInput) {
    if (!purchaseDate || !(addressData.street || street)) {
      setPurchaseHistoryStatus("Enter the property address and approximate purchase date.");
      return;
    }
    try {
      setPurchaseHistoryStatus("Checking the property value around the purchase date...");
      const address1 = addressData.street || street;
      const address2 = [addressData.city || city, [addressData.state || stateName, addressData.zip || zip].filter(Boolean).join(" ")].filter(Boolean).join(", ");
      const res = await fetch("/api/property-history", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ address: addressData.address || [address1, address2].filter(Boolean).join(", "), address1, street: address1, city: addressData.city || city, state: addressData.state || stateName, zip: addressData.zip || zip, address2, purchaseDate, currentValue: homeValue }) });
      const data = await res.json();
      const value = Number(data?.value || 0);
      if (value > 0) {
        setPurchasePeriodValueInput(String(Math.round(value)));
        setPurchaseHistoryStatus(`${data?.exactSale ? "Recorded purchase price" : "Estimated value around purchase date"}: ${money(value)}. You can adjust it if needed.`);
      } else {
        setPurchaseHistoryStatus("We could not confirm a reliable recorded purchase price near that date. Please enter the approximate purchase price below.");
      }
    } catch {
      setPurchaseHistoryStatus("We could not confirm a reliable recorded purchase price near that date. Please enter the approximate purchase price below.");
    }
  ...
  function selectAddress(result: AddressResult) {
    ...
    const selectedAddress = { address: result.label, street: selectedStreet, city: selectedCity, state: selectedState, zip: selectedZip };
    lookupHomeValue(selectedAddress);
    if (purchaseDateInput) lookupPurchasePeriodValue(selectedAddress, purchaseDateInput);
  }

  function chooseProduct(next: ProductKey) {
    setProduct(next); setMainGoal("");
    if (next !== "purchase") setPurchaseTargetMode(false);
    if (next === "payment_check") setPaymentCompareOpen(false);
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#050505] text-white">
      ...
      <form onSubmit={submitLead} noValidate className="mx-auto max-w-6xl px-3 pb-16 pt-4 sm:px-6 lg:pt-8">
        ...
        <input type="hidden" name="payment_check_estimated_payment" value={product === "payment_check" ? paymentCheckEstimatedPayment : ""} />
        <input type="hidden" name="payment_check_monthly_difference" value={product === "payment_check" ? paymentCheckMonthlyDifference : ""} />
        <input type="hidden" name="payment_check_result" value={product === "payment_check" ? (paymentCheckCanImprove ? "Potential monthly savings identified" : "Current payment appears competitive") : ""} />
        <input type="hidden" name="payment_check_current_payment" value={product === "payment_check" ? paymentCheckCurrentPayment : ""} />
        <input type="hidden" name="payment_check_mortgage_term" value={product === "payment_check" ? mortgageTermInput : ""} />
        <input type="hidden" name="payment_check_purchase_date" value={product === "payment_check" ? purchaseDateInput : ""} />
        <input type="hidden" name="payment_check_purchase_period_value" value={product === "payment_check" ? purchasePeriodValueInput : ""} />
        ...
        {product === "payment_check" ? (
          <section id="step1" className="mt-5 rounded-[28px] border border-amber-300/20 bg-[#11100b]/88 p-4 sm:p-7">
            <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
              <div><div className="text-[11px] font-black uppercase tracking-[.28em] text-amber-200">Mortgage Payment Check</div><h2 className="mt-2 text-3xl font-black tracking-[-.05em] sm:text-5xl">See if you may be paying too much</h2><p className="mt-3 max-w-3xl text-sm font-semibold leading-relaxed text-white/60 sm:text-base">Enter your property and current loan details. We will estimate your present principal-and-interest payment and compare it with a possible new option.</p></div>
              <div className="rounded-full border border-amber-300/25 bg-amber-300/10 px-4 py-3 text-sm font-black text-amber-100">Private estimate • No credit check</div>
            </div>
            <div className="mt-5 grid gap-4 lg:grid-cols-[.98fr_1.02fr]">
              <div className="rounded-[26px] border border-white/15 bg-white/[.035] p-4 sm:p-5">
                <div className="text-[11px] font-black uppercase tracking-[.28em] text-amber-200">You Enter</div><h3 className="mt-2 text-2xl font-black">Your property and current mortgage</h3>
                <div className="mt-4 grid gap-3">
                  <div className="rounded-3xl border border-amber-300/20 bg-black/20 p-4"><label className="text-sm font-black text-white/80">Property address</label><div className="relative mt-2"><input value={street} onChange={(e) => searchAddress(e.target.value)} placeholder="Search your property address" className="field" />{addressResults.length > 0 && <div className="absolute z-30 mt-2 max-h-64 w-full overflow-auto rounded-2xl border border-amber-300/20 bg-[#11100b] p-2 shadow-2xl">{addressResults.map((r, i) => <button key={`${r.label}-${i}`} type="button" onClick={() => selectAddress(r)} className="block w-full rounded-xl px-3 py-3 text-left text-sm font-bold text-white/85 hover:bg-amber-300/10">{r.label}</button>)}</div>}</div><div className="mt-3 grid gap-2 sm:grid-cols-[1fr_90px_100px]"><input name="city_payment_check" value={city} onChange={(e) => setCity(e.target.value)} placeholder="City" className="field" /><input name="state_payment_check" value={stateName} onChange={(e) => setStateName(e.target.value)} placeholder="State" className="field" /><input name="zip_payment_check" value={zip} onChange={(e) => setZip(e.target.value)} placeholder="ZIP" className="field" /></div><p className="input-help">{valueStatus || addressStatus}</p></div>
                  <div className="grid gap-3">
                    <label className="premium-input-card"><div className="input-header"><span>Current Loan</span><b>Original mortgage term</b></div><select name="payment_check_term_visible" value={mortgageTermInput} onChange={(e) => setMortgageTermInput(e.target.value)} className="field mt-3"><option value="10">10 years</option><option value="15">15 years</option><option value="20">20 years</option><option value="25">25 years</option><option value="30">30 years</option><option value="40">40 years</option></select><p className="input-help">Choose the original term of your current mortgage.</p></label>
                    <label className="premium-input-card"><div className="input-header"><span>Property History</span><b>Approximate purchase date</b></div><input type="date" name="payment_check_purchase_date_visible" value={purchaseDateInput} max={new Date().toISOString().slice(0,10)} onChange={(e) => { const next=e.target.value; setPurchaseDateInput(next); if (street.trim()) lookupPurchasePeriodValue({ street, city, state: stateName, zip, address: `${street}, ${city}, ${stateName} ${zip}` }, next); }} className="field mt-3" /><p className="input-help">Used to estimate the remaining loan term and look up the purchase-period value.</p></label>
                  </div>
                  <MoneyInput label="Current loan payoff amount" name="mortgage_balance" value={mortgageBalanceInput} onChange={setMortgageBalanceInput} help="Enter the approximate principal amount still required to pay off the current mortgage." />
                  <div className="premium-input-card"><div className="input-header"><span>Current Loan</span><b>Current interest rate</b></div><div className="money-shell"><input value={currentRateInput} onChange={(e) => setCurrentRateInput(e.target.value)} name="current_interest_rate_visible" className="premium-money-input" placeholder="6.75" inputMode="decimal" /><em>%</em></div><p className="input-help">Enter the interest rate shown on your current mortgage statement.</p></div>
                  <div className="premium-input-card"><div className="input-header"><span>Property History</span><b>Value around the purchase date</b></div><div className="money-shell"><em>$</em><input value={purchasePeriodValueInput} onChange={(e) => setPurchasePeriodValueInput(e.target.value)} name="payment_check_purchase_period_value_visible" className="premium-money-input" placeholder="Auto-filled when available" inputMode="decimal" /></div><p className="input-help">{purchaseHistoryStatus}</p></div>
                  <div className="rounded-3xl border border-amber-300/20 bg-amber-300/[.045] p-4"><label className="text-[11px] font-black uppercase tracking-[.22em] text-amber-200">Current credit score range</label><select name="calculator_credit_score_range_visible" value={calculatorCreditRange} onChange={(e) => setCalculatorCreditRange(e.target.value)} className="field mt-3"><option value="below_550">Below 550</option><option value="550_619">550–619</option><option value="620_679">620–679</option><option value="680_719">680–719</option><option value="720_plus">720+</option></select><p className="mt-2 text-sm font-semibold text-white/45">Used only to improve this estimate. No credit report is pulled.</p></div>
                </div>
              </div>
              <div className="payment-check-result">
                <div className="text-[11px] font-black uppercase tracking-[.28em] text-amber-200">Your Result</div><h3 className="mt-2 text-2xl font-black">Your mortgage payment review</h3>
                {!paymentCheckComplete ? (
                  <div className="payment-check-wait"><span>Complete your mortgage details</span><p>Add the property address, mortgage term, purchase date, payoff amount, interest rate, and credit range. Your result will update automatically.</p></div>
                ) : paymentCheckCanImprove ? (
                  <div className="payment-check-positive"><div className="payment-check-badge">GREAT NEWS — SAVINGS FOUND</div><h4>We may have a mortgage company that can beat your current estimated payment.</h4><div className="payment-check-grid"><ResultCard label="Your Current Payment" value={`${money(paymentCheckCurrentPayment)}/mo`} /><ResultCard label="Estimated New Payment" value={`${money(paymentCheckEstimatedPayment)}/mo`} highlight /><ResultCard label="Approximate Monthly Savings" value={`${money(paymentCheckMonthlyDifference)}/mo`} highlight /><ResultCard label="Approximate Annual Savings" value={money(paymentCheckMonthlyDifference * 12)} /></div><button type="button" onClick={() => document.getElementById("client-information")?.scrollIntoView({ behavior: "smooth", block: "start" })} className="beat-rate-btn">Beat My Current Rate →</button></div>
                ) : (
                  <div className="payment-check-competitive"><div className="payment-check-badge safe">GREAT NEWS — YOUR PAYMENT LOOKS COMPETITIVE</div><h4>Your current estimated mortgage payment already appears competitive.</h4><div className="payment-check-grid"><ResultCard label="Your Current Payment" value={`${money(paymentCheckCurrentPayment)}/mo`} /><ResultCard label="Estimated New Payment" value={`${money(paymentCheckEstimatedPayment)}/mo`} highlight /></div><p>Based on the details entered, we do not currently estimate a meaningful payment reduction. Your existing mortgage appears to be in a good position, but you may still request a professional review of other options.</p><button type="button" onClick={() => document.getElementById("client-information")?.scrollIntoView({ behavior: "smooth", block: "start" })} className="beat-rate-btn secondary">Review My Options →</button></div>
                )}
                <p className="payment-check-disclaimer">Approximate principal-and-interest comparison only—not a loan offer or approval. Taxes, insurance, mortgage insurance, fees, loan type, actual origination date, and lender terms can change the final payment.</p>
              </div>
            </div>
          </section>
        ) : (
...
.payment-check-result{border-radius:26px;border:1px solid rgba(246,193,90,.25);background:linear-gradient(180deg,rgba(19,17,11,.96),rgba(5,5,5,.96));padding:20px;box-shadow:inset 0 0 35px rgba(246,193,90,.05),0 24px 60px rgba(0,0,0,.28)}.payment-check-wait,.payment-check-positive,.payment-check-competitive{margin-top:16px;border-radius:24px;border:1px solid rgba(255,255,255,.10);background:rgba(255,255,255,.035);padding:20px}.payment-check-wait span{font-size:22px;font-weight:1000;color:#f7d982}.payment-check-wait p,.payment-check-competitive p{margin:8px 0 0;color:rgba(255,255,255,.65);font-size:14px;font-weight:750;line-height:1.55}.payment-check-badge{display:inline-flex;align-items:center;justify-content:center;border-radius:999px;border:1px solid rgba(134,239,172,.78);background:linear-gradient(135deg,rgba(22,163,74,.95),rgba(34,197,94,.72));padding:11px 17px;color:#fff;font-size:11px;font-weight:1000;letter-spacing:.18em;box-shadow:0 0 0 4px rgba(34,197,94,.10),0 0 30px rgba(34,197,94,.30);text-shadow:0 1px 2px rgba(0,0,0,.35)}.payment-check-badge.safe{border-color:rgba(255,226,151,.75);background:linear-gradient(135deg,#b7791f,#e0b84e);color:#090806;box-shadow:0 0 0 4px rgba(246,193,90,.10),0 0 30px rgba(246,193,90,.25);text-shadow:none}.payment-check-positive h4,.payment-check-competitive h4{margin:13px 0 0;font-size:clamp(22px,3vw,32px);font-weight:1000;letter-spacing:-.04em;line-height:1.05}.payment-check-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px;margin-top:18px}.beat-rate-btn{width:100%;margin-top:18px;border-radius:20px;border:1px solid rgba(255,255,255,.42);background:linear-gradient(180deg,#fff3b7,#d4af37 55%,#a97819);padding:16px 18px;color:#060606;font-size:14px;font-weight:1000;letter-spacing:.08em;text-transform:uppercase;box-shadow:0 0 0 3px rgba(246,193,90,.10),0 16px 34px rgba(0,0,0,.32)}.beat-rate-btn.secondary{background:linear-gradient(180deg,#f7e7b1,#c99c39);}.payment-check-disclaimer{margin:16px 2px 0;color:rgba(255,255,255,.42);font-size:11px;font-weight:700;line-height:1.45}@media(max-width:640px){.premium-input-card{padding:16px;border-radius:20px}.input-header b{font-size:13px}.input-help{font-size:12px}.field{padding:14px 14px;font-size:15px}.premium-money-input{font-size:28px}.payment-check-result{padding:14px}.payment-check-grid{grid-template-columns:1fr}.payment-check-positive h4,.payment-check-competitive h4{font-size:23px}.service-payment_check .tile-title{font-size:12px!important}}.mascot-stage{position:relative;display:flex;align-items:center;justify-content:center;min-width:0;min-height:300px;overflow:visible;isolation:isolate;background:transparent;padding:8px}.mascot-stage:before{content:"";position:absolute;left:50%;top:50%;width:78%;height:76%;transform:translate(-50%,-50%);border-radius:50%;background:radial-gradient(circle,rgba(255,226,151,.34) 0%,rgba(212,175,55,.20) 31%,rgba(212,175,55,.08) 52%,transparent 72%);filter:blur(25px);animation:mascotGlow 3.8s ease-in-out infinite;z-index:0;pointer-events:none}.mascot-glow{position:absolute;left:50%;top:52%;width:68%;height:64%;transform:translate(-50%,-50%);border-radius:50%;background:radial-gradient(circle,rgba(255,241,198,.22),rgba(212,175,55,.10) 42%,transparent 72%);filter:blur(18px);z-index:0;pointer-events:none}.mascot-video{position:relative;z-index:2;display:block;width:min(88%,390px);height:auto;max-height:360px;object-fit:contain;filter:drop-shadow(0 22px 28px rgba(0,0,0,.52)) drop-shadow(0 0 24px rgba(212,175,55,.16));background:transparent;pointer-events:none}@keyframes mascotGlow{0%,100%{opacity:.72;transform:translate(-50%,-50%) scale(.97)}50%{opacity:1;transform:translate(-50%,-50%) scale(1.035)}}@media(max-width:759px){.hero-grid{display:grid!important;grid-template-columns:minmax(0,1.15fr) minmax(118px,.85fr)!important;align-items:center!important;gap:8px!important}.hero-copy{min-width:0}.hero-copy h1{font-size:clamp(27px,8vw,37px)!important;line-height:.98!important}.hero-copy p{font-size:12.5px!important;line-height:1.42!important}.trust-row{gap:5px!important}.trust-row span{font-size:8.5px!important;padding:7px 8px!important}.mascot-stage{min-height:245px!important;padding:0!important;align-self:stretch}.mascot-video{width:115%!important;max-width:260px!important;max-height:285px!important;transform:translateX(-3%)}.mascot-stage:before{width:110%;height:78%;filter:blur(18px)}.mascot-glow{width:95%;height:66%;filter:blur(14px)}}@media(max-width:390px){.hero-grid{grid-template-columns:minmax(0,1.2fr) minmax(108px,.8fr)!important;gap:4px!important}.hero-copy h1{font-size:28px!important}.hero-copy p{font-size:11.5px!important}.mascot-stage{min-height:225px!important}.mascot-video{width:122%!important;max-width:225px!important;max-height:255px!important;transform:translateX(-5%)}}
      `}</style>
    </main>
  );
}
...
function MoneyInput({ label, name, value, onChange, help }: { label: string; name: string; value: string; onChange: (v: string) => void; help: string }) {
  return <div className="premium-input-card"><div className="input-header"><span>Client Input</span><b>{label}</b></div><div className="money-shell"><em>$</em><input name={name} value={formatMoneyInput(value)} onChange={(e) => onChange(e.target.value.replace(/[^0-9]/g, ""))} className="premium-money-input" placeholder="" inputMode="decimal" /></div><p className="input-help">{help}</p></div>;
}
...
function ResultCard({ label, value, highlight = false }: { label: string; value: string; highlight?: boolean }) {
  return <div className={`rounded-3xl border p-4 shadow-[inset_0_1px_rgba(255,255,255,.06)] ${highlight ? "border-amber-300/30 bg-gradient-to-br from-amber-300/14 to-amber-300/6" : "border-white/10 bg-black/24"}`}><div className="text-[11px] font-black uppercase tracking-[.16em] text-white/62">{label}</div><div className={`mt-2 text-2xl font-black tracking-[-.04em] ${highlight ? "text-amber-300" : "text-white"}`}>{value}</div></div>;
}
