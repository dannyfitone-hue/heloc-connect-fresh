...
export default function LandingPage() {
  ...
  const extraLoansTotal = extraLoanInputs.slice(0, Math.max(0, loanCount - 1)).reduce((sum, v) => sum + toNumber(v), 0);
      ...
      <form onSubmit={submitLead} className="mx-auto max-w-6xl px-3 pb-16 pt-4 sm:px-6 lg:pt-8">
        ...
        <section id="step1" className="mt-5 rounded-[28px] border border-emerald-300/20 bg-[#071522]/88 p-4 sm:p-7">
          ...
          <div className="mt-5 grid gap-4 lg:grid-cols-[.9fr_1.1fr]"><div className="rounded-[26px] border border-white/15 bg-white/[.035] p-4 sm:p-5"><div className="text-[11px] font-black uppercase tracking-[.28em] text-cyan-200">You Enter</div><h3 className="mt-2 text-2xl font-black">Information needed from you</h3><div className="mt-4 grid gap-3">
                ...
                {product !== "purchase" && <MoneyInput label={loanCount > 1 ? "Loan 1 / first mortgage balance" : "Current mortgage balance"} name="mortgage_balance" value={mortgageBalanceInput} onChange={setMortgageBalanceInput} help="Approximate balance remaining on your first mortgage." />}
                {product !== "purchase" && extraLoanInputs.slice(0, Math.max(0, loanCount - 1)).map((value, index) => (
                  <MoneyInput key={index} label={`Loan ${index + 2} balance`} name={`existing_loan_${index + 2}_balance`} value={value} onChange={(next) => setExtraLoanInputs((prev) => prev.map((item, i) => i === index ? next : item))} help="Add the remaining balance for this additional mortgage, HELOC, or property-secured loan." />
                ))}
