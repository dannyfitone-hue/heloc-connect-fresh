...
export default function LandingPage() {
  ...
  const [showPaymentCompare, setShowPaymentCompare] = useState(false);
  ...
  const refinanceTotalMonthlyPayment = refinancePayment;
  const paymentDifference = Math.abs(helocTotalMonthlyPayment - refinanceTotalMonthlyPayment);
  const paymentWinner = helocTotalMonthlyPayment && refinanceTotalMonthlyPayment ? (helocTotalMonthlyPayment < refinanceTotalMonthlyPayment ? "HELOC" : refinanceTotalMonthlyPayment < helocTotalMonthlyPayment ? "Refinance" : "Tie") : "";
  ...
  function chooseProduct(next: ProductKey) {
    setProduct(next); setMainGoal(""); setShowPaymentCompare(false);
    ...
    <main className="min-h-screen overflow-x-hidden bg-[#050505] text-white">
      ...
      <header className="sticky top-0 z-50 border-b border-amber-200/10 bg-[#050505]/95 backdrop-blur-xl">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-3">
          <a href="/" className="flex items-center" aria-label="HELOC CONNECT home">
            <img src="/heloc-connect-logo-full.png" alt="HELOC CONNECT" className="h-14 w-auto object-contain sm:h-16" />
          </a>
      ...
      <form onSubmit={submitLead} className="mx-auto max-w-6xl px-3 pb-16 pt-4 sm:px-6 lg:pt-8">
        ...
        <input type="hidden" name="heloc_total_monthly_payment" value={helocTotalMonthlyPayment} />
        <input type="hidden" name="refinance_total_monthly_payment" value={refinanceTotalMonthlyPayment} />
        <input type="hidden" name="lower_payment_option" value={paymentWinner} />
        ...
        <section className="process-shell">
          <div className="topbar2">
            <div className="brandline brandline-logo"><img src="/heloc-connect-logo-full.png" alt="HELOC CONNECT" /></div>
        ...
        <section id="step1" className="mt-5 rounded-[28px] border border-amber-300/20 bg-[#11100b]/88 p-4 sm:p-7">
          ...
          <div className="mt-5 grid gap-4 lg:grid-cols-[.9fr_1.1fr]"><div className="rounded-[26px] border border-white/15 bg-white/[.035] p-4 sm:p-5"><div className="text-[11px] font-black uppercase tracking-[.28em] text-amber-200">You Enter</div><h3 className="mt-2 text-2xl font-black">Information needed from you</h3><div className="mt-4 grid gap-3">
              ...
              </div></div><div className="rounded-[26px] border border-amber-300/20 bg-amber-300/8 p-4 sm:p-5"><div className="text-[11px] font-black uppercase tracking-[.28em] text-amber-200">We Calculate</div><h3 className="mt-2 text-2xl font-black">Your estimated preview</h3><div className="mt-4 grid grid-cols-2 gap-3"><ResultCard label="Property Value" value={homeValue ? money(homeValue) : "Waiting"} />{product !== "purchase" && <ResultCard label="Total Existing Loans" value={money(mortgageBalance)} />}{product !== "purchase" && <ResultCard label="Estimated Equity Room" value={maxEquity ? money(maxEquity) : "$0"} />}{product === "refinance" && <ResultCard label="Estimated Current Payment" value={`${money(currentMortgagePayment)}/mo`} />}{product === "refinance" && <ResultCard label="New Loan With Cash" value={money(refinanceLoan)} highlight />}{product === "purchase" && <ResultCard label="Purchase Loan" value={money(purchaseLoan)} />}<ResultCard label={product === "refinance" ? "Estimated New Payment" : "Estimated Payment"} value={`${money(estimatedPayment)}/mo`} highlight />{product !== "purchase" && <ResultCard label="Cash Requested" value={money(cashToUse)} highlight />}</div><p className="mt-4 rounded-2xl border border-amber-300/15 bg-black/20 p-3 text-sm font-semibold leading-relaxed text-white/68">{product === "refinance" ? "Refinance preview totals every existing property loan, adds the cash requested, then estimates one new combined payment." : product === "purchase" ? "Purchase preview estimates a monthly payment based on the selected loan amount." : "HELOC / equity-line preview subtracts all existing property loans from the home value to estimate available equity room."}</p>{product !== "purchase" && <div className="mt-4 rounded-[26px] border border-amber-300/25 bg-gradient-to-br from-amber-300/10 via-black/28 to-black/45 p-4 shadow-[inset_0_1px_rgba(255,255,255,.07),0_18px_44px_rgba(0,0,0,.22)]"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><div className="text-[10px] font-black uppercase tracking-[.22em] text-amber-200">Payment Decision</div><div className="mt-1 text-lg font-black tracking-[-.03em] text-white">Compare total monthly payments</div><p className="mt-1 text-sm font-semibold leading-relaxed text-white/58">See HELOC total payment vs refinance total payment in one clean view.</p></div><button type="button" onClick={() => setShowPaymentCompare((v) => !v)} className="rounded-full border border-amber-200/35 bg-gradient-to-r from-amber-200 to-amber-500 px-5 py-3 text-sm font-black uppercase tracking-[.14em] text-black shadow-[0_10px_32px_rgba(246,193,90,.22)] transition hover:scale-[1.02]">{showPaymentCompare ? "Hide Compare" : "Compare Payments"}</button></div>{showPaymentCompare && <div className="mt-4 grid gap-3 sm:grid-cols-2"><div className="rounded-3xl border border-white/10 bg-black/32 p-4"><div className="text-[10px] font-black uppercase tracking-[.18em] text-white/42">HELOC Option</div><div className="mt-3 space-y-2 text-sm font-bold text-white/68"><div className="flex items-center justify-between gap-3"><span>Current mortgage payment</span><b className="text-white">{money(currentMortgagePayment)}/mo</b></div><div className="flex items-center justify-between gap-3"><span>New HELOC payment</span><b className="text-amber-200">{money(helocPayment)}/mo</b></div></div><div className="mt-4 border-t border-white/10 pt-4"><div className="text-[11px] font-black uppercase tracking-[.16em] text-amber-200">Total Monthly Payment</div><div className="mt-1 text-3xl font-black tracking-[-.05em] text-white">{money(helocTotalMonthlyPayment)}/mo</div></div></div><div className="rounded-3xl border border-amber-300/30 bg-amber-300/10 p-4"><div className="text-[10px] font-black uppercase tracking-[.18em] text-white/42">Refinance Option</div><div className="mt-3 space-y-2 text-sm font-bold text-white/68"><div className="flex items-center justify-between gap-3"><span>New refinance mortgage payment</span><b className="text-amber-200">{money(refinanceTotalMonthlyPayment)}/mo</b></div><div className="flex items-center justify-between gap-3"><span>Includes existing loans + requested cash</span><b className="text-white">{money(refinanceLoan)}</b></div></div><div className="mt-4 border-t border-white/10 pt-4"><div className="text-[11px] font-black uppercase tracking-[.16em] text-amber-200">Total Monthly Payment</div><div className="mt-1 text-3xl font-black tracking-[-.05em] text-white">{money(refinanceTotalMonthlyPayment)}/mo</div></div></div><div className="sm:col-span-2 rounded-3xl border border-emerald-300/20 bg-emerald-300/8 p-4 text-center"><div className="text-[10px] font-black uppercase tracking-[.2em] text-emerald-200">In One Look</div><div className="mt-1 text-xl font-black text-white">{paymentWinner === "Tie" ? "Both options show the same estimated monthly payment." : paymentWinner ? `${paymentWinner} shows the lower estimated monthly payment by ${money(paymentDifference)}/mo.` : "Enter home value, existing mortgage balance, current rate, and cash requested to compare accurately."}</div><p className="mt-2 text-xs font-semibold leading-relaxed text-white/55">Estimates are for quick comparison only. A matched mortgage company can review final numbers, terms, and program options.</p></div></div>}</div>}</div></div>
        </section>
