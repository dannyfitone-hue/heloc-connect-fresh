...
export default function LandingPage() {
  ...
  const loanDetails = [
    { balance: toNumber(mortgageBalanceInput), rate: Number(currentRateInput || 0) / 100, payment: toNumber(currentPaymentInput) },
    ...extraLoanInputs.slice(0, visibleExtraLoanCount).map((balance, index) => ({ balance: toNumber(balance), rate: Number(extraLoanRateInputs[index] || 0) / 100, payment: 0 }))
  ];
  ...
  const currentMortgagePayment = useMemo(() => loanDetails.reduce((sum, loan) => {
    if (loan.payment && loan.payment > 0) return sum + loan.payment;
    if (loan.rate && loan.rate > 0) return sum + monthlyPayment(loan.balance, loan.rate);
    return sum;
  }, 0), [mortgageBalanceInput, currentRateInput, currentPaymentInput, extraLoanInputs, extraLoanRateInputs, loanCountInput]);
  ...
  function togglePaymentCompare() {
    ...
    const paymentReady = toNumber(currentPaymentInput) > 0 || toNumber(currentRateInput) > 0;
    const refinanceReady = requiredFieldsFor("refinance");
    if (!paymentReady) {
      setMissingFields((prev) => Array.from(new Set([...prev, "currentPayment"])));
      setQuickCompareNotice("Enter your current monthly mortgage payment so we can compare the HELOC total payment correctly.");
      setTimeout(() => {
      ...
      <form onSubmit={submitLead} className="mx-auto max-w-6xl px-3 pb-16 pt-4 sm:px-6 lg:pt-8">
        ...
        <section id="step1" className="mt-5 rounded-[28px] border border-amber-300/20 bg-[#11100b]/88 p-4 sm:p-7">
          ...
          <div className="mt-5 grid gap-4 lg:grid-cols-[.9fr_1.1fr]"><div className="rounded-[26px] border border-white/15 bg-white/[.035] p-4 sm:p-5"><div className="text-[11px] font-black uppercase tracking-[.28em] text-amber-200">You Enter</div><h3 className="mt-2 text-2xl font-black">Information needed from you</h3><div className="mt-4 grid gap-3">
                ...
                {product !== "purchase" && <div id="calc-currentPayment" className={wrapperClass("currentPayment", "premium-input-card")}><div className="input-header"><span>Payment Compare</span><b>Current monthly mortgage payment</b></div><div className="money-shell"><em>$</em><input value={currentPaymentInput} onChange={(e) => setCurrentPaymentInput(e.target.value)} name="current_mortgage_monthly_payment" className="premium-money-input" placeholder="" inputMode="decimal" /></div><p className="input-help">Used for the HELOC comparison: current mortgage payment plus estimated new HELOC payment.</p></div>}
