...
export default function LandingPage() {
  ...
  const paymentCheckEstimatedPayment = useMemo(() => monthlyPayment(toNumber(mortgageBalanceInput), offerRate), [mortgageBalanceInput, offerRate]);
  const paymentCheckMonthlyDifference = Math.round(currentMonthlyPaymentEntered - paymentCheckEstimatedPayment);
  const paymentCheckCanImprove = toNumber(mortgageBalanceInput) > 0 && currentMonthlyPaymentEntered > 0 && paymentCheckMonthlyDifference >= 50;
  const paymentCheckComplete = toNumber(mortgageBalanceInput) > 0 && currentMonthlyPaymentEntered > 0 && toNumber(currentRateInput) > 0;
        ...
        {product === "payment_check" ? (
          <section id="step1" className="mt-5 rounded-[28px] border border-amber-300/20 bg-[#11100b]/88 p-4 sm:p-7">
            <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
              <div><div className="text-[11px] font-black uppercase tracking-[.28em] text-amber-200">Mortgage Payment Check</div><h2 className="mt-2 text-3xl font-black tracking-[-.05em] sm:text-5xl">See if your current payment is competitive</h2><p className="mt-3 max-w-3xl text-sm font-semibold leading-relaxed text-white/60 sm:text-base">Enter four details from your current mortgage. We will compare your payment with the same internal estimate used by the HELOC CONNECT calculator.</p></div>
            ...
            <div className="mt-5 grid gap-4 lg:grid-cols-[.92fr_1.08fr]">
              <div className="rounded-[26px] border border-white/15 bg-white/[.035] p-4 sm:p-5">
                <div className="text-[11px] font-black uppercase tracking-[.28em] text-amber-200">You Enter</div><h3 className="mt-2 text-2xl font-black">Your current mortgage</h3>
                <div className="mt-4 grid gap-3">
                  <MoneyInput label="Current mortgage balance" name="mortgage_balance" value={mortgageBalanceInput} onChange={setMortgageBalanceInput} help="Enter the approximate remaining principal balance." />
                  <div className="premium-input-card"><div className="input-header"><span>Current Loan</span><b>Current monthly mortgage payment</b></div><div className="money-shell"><em>$</em><input value={currentPaymentInput} onChange={(e) => setCurrentPaymentInput(e.target.value)} name="current_mortgage_monthly_payment" className="premium-money-input" placeholder="3500" inputMode="decimal" /></div><p className="input-help">Enter your current monthly payment. This comparison is an estimate and may not include the same taxes, insurance, fees, or remaining term.</p></div>
                  ...
                  <div className="rounded-3xl border border-amber-300/20 bg-amber-300/[.045] p-4"><label className="text-[11px] font-black uppercase tracking-[.22em] text-amber-200">Current credit score range</label><select name="calculator_credit_score_range_visible" value={calculatorCreditRange} onChange={(e) => setCalculatorCreditRange(e.target.value)} className="field mt-3"><option value="below_550">Below 550</option><option value="550_619">550–619</option><option value="620_679">620–679</option><option value="680_719">680–719</option><option value="720_plus">720+</option></select><p className="mt-2 text-sm font-semibold text-white/45">Used only to improve this payment estimate. No credit report is pulled.</p></div>
                </div>
              </div>
              <div className="payment-check-result">
                <div className="text-[11px] font-black uppercase tracking-[.28em] text-amber-200">Your Result</div><h3 className="mt-2 text-2xl font-black">Mortgage payment review</h3>
                {!paymentCheckComplete ? (
                  <div className="payment-check-wait"><span>Enter all four details</span><p>Your result will update automatically as soon as the mortgage balance, payment, rate, and credit range are complete.</p></div>
                ) : paymentCheckCanImprove ? (
                  <div className="payment-check-positive"><div className="payment-check-badge">GREAT NEWS</div><h4>We may have a mortgage company that can improve your current payment.</h4><div className="payment-check-grid"><ResultCard label="Your Current Payment" value={`${money(currentMonthlyPaymentEntered)}/mo`} /><ResultCard label="Estimated Comparable Payment" value={`${money(paymentCheckEstimatedPayment)}/mo`} highlight /><ResultCard label="Possible Monthly Savings" value={`${money(paymentCheckMonthlyDifference)}/mo`} highlight /><ResultCard label="Possible Annual Savings" value={money(paymentCheckMonthlyDifference * 12)} /></div><button type="button" onClick={() => document.getElementById("client-information")?.scrollIntoView({ behavior: "smooth", block: "start" })} className="beat-rate-btn">Beat My Current Rate →</button></div>
                ) : (
                  <div className="payment-check-competitive"><div className="payment-check-badge safe">COMPETITIVE PAYMENT</div><h4>Great news—your current mortgage payment appears competitive based on the information you entered.</h4><div className="payment-check-grid"><ResultCard label="Your Current Payment" value={`${money(currentMonthlyPaymentEntered)}/mo`} /><ResultCard label="Estimated Comparable Payment" value={`${money(paymentCheckEstimatedPayment)}/mo`} highlight /></div><p>Your current mortgage appears to be in a strong position. We do not currently estimate a meaningful monthly-payment improvement, but we can still review other mortgage or home-equity options.</p><button type="button" onClick={() => document.getElementById("client-information")?.scrollIntoView({ behavior: "smooth", block: "start" })} className="beat-rate-btn secondary">Review My Options →</button></div>
                )}
                <p className="payment-check-disclaimer">For comparison purposes only—not a loan offer or approval. Actual payments depend on term, taxes, insurance, fees, loan type, property details, and the matched mortgage company’s final terms.</p>
              </div>
            </div>
          </section>
        ) : (
