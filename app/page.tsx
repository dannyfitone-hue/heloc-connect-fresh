...
const products: Array<{
  ...
  { key: "equity_card", eyebrow: "CREDIT CARD", title: "CREDIT CARD", shortTitle: "CREDIT CARD", short: "Use your home equity as spending limit. Use as you need, just like a credit card but with much lower APR.", icon: "▭", accent: "emerald" },
...
export default function LandingPage() {
  ...
  const [missingPreviewFields, setMissingPreviewFields] = useState<string[]>([]);
  const [previewNotice, setPreviewNotice] = useState("");
  const [voicePlayed, setVoicePlayed] = useState(false);
  const welcomeAudioRef = useRef<HTMLAudioElement | null>(null);
  const welcomeVoiceStateRef = useRef<"idle" | "playing" | "played">("idle");

  function speakWelcome(force = false) {
    if (typeof window === "undefined") return false;

    if (!force && welcomeVoiceStateRef.current !== "idle") {
      return false;
    }
    ...
    try {
      ...
      if (!audio) {
        ...
        audio = existing || new Audio("/audio/welcome-voice.mp3");
        audio.preload = "auto";
        audio.volume = 0.95;
        ...
        audio.addEventListener("ended", () => {
          welcomeVoiceStateRef.current = "played";
          setVoicePlayed(true);
        });

        audio.addEventListener("pause", () => {
          if (welcomeVoiceStateRef.current === "playing" && audio && audio.currentTime > 0 && !audio.ended) {
            // Do not reset to the beginning if the browser briefly pauses the file.
            welcomeVoiceStateRef.current = "playing";
          }
        });
      }
      ...
      if (force) {
        ...
        audio.currentTime = 0;
        welcomeVoiceStateRef.current = "idle";
      }

      if (welcomeVoiceStateRef.current !== "idle") {
        return false;
      }
      ...
      if (playPromise && typeof playPromise.then === "function") {
        playPromise.catch((error) => {
          ...
          // Keep state idle so the next permitted user action can start the same file once.
          welcomeVoiceStateRef.current = "idle";
  ...
  useEffect(() => {
    ...
    const tryPlayOnce = () => {
      if (welcomeVoiceStateRef.current === "idle") speakWelcome(false);
    };
    const playWhenVisible = () => {
      if (!document.hidden && welcomeVoiceStateRef.current === "idle") speakWelcome(false);
    };

    const attempts = [450, 1100, 2200].map((delay) => window.setTimeout(tryPlayOnce, delay));

    window.addEventListener("load", tryPlayOnce, { once: true });
    window.addEventListener("focus", tryPlayOnce, { once: true });
    ...
    return () => {
      ...
      window.removeEventListener("load", tryPlayOnce);
      window.removeEventListener("focus", tryPlayOnce);
  ...
  function requiredFieldsForPreview(target: ProductKey) {
    const missing: string[] = [];
    if (target !== "purchase") {
      if (!street.trim() && !homeValue) missing.push("property_address_search");
      if (!homeValue) missing.push("home_value_visible");
      if (!toNumber(mortgageBalanceInput)) missing.push("mortgage_balance");
      for (let i = 0; i < visibleExtraLoanCount; i += 1) {
        if (!toNumber(extraLoanInputs[i])) missing.push(`existing_loan_${i + 2}_balance`);
      }
      if (!toNumber(requestedCashInput)) missing.push("requested_cash");
      if (target === "refinance") {
        if (!currentRateInput.trim()) missing.push("current_interest_rate_visible");
        for (let i = 0; i < visibleExtraLoanCount; i += 1) {
          if (!extraLoanRateInputs[i]?.trim()) missing.push(`existing_loan_${i + 2}_interest_rate`);
        }
      }
    }
    return missing;
  ...
  function scrollToMissingField(fields: string[]) {
    const first = fields[0];
    if (!first) return;
    setTimeout(() => {
      const el = document.querySelector(`[name="${first}"]`) as HTMLElement | null;
      const target = el?.closest(".premium-input-card, .calculator-address-card, .property-value-card") || el;
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#050505] text-white">
      <audio id="hc-welcome-autoplay" src="/audio/welcome-voice.mp3" preload="auto" playsInline className="hidden" />
      ...
      <header className="sticky top-0 z-50 border-b border-amber-200/10 bg-[#050505]/95 backdrop-blur-xl">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={() => speakWelcome(true)} aria-label="Replay welcome voice" title="Replay welcome voice" className={`grid h-10 w-10 place-items-center rounded-full border text-sm font-black transition ${voicePlayed ? "border-amber-300/35 bg-amber-300/10 text-amber-100" : "border-white/15 bg-white/[.045] text-white/75 hover:border-amber-300/35 hover:text-amber-100"}`}>♫</button>
          </div>
        </div>
      </header>
        ...
        <section id="step1" className="mt-5 rounded-[28px] border border-amber-300/20 bg-[#11100b]/88 p-4 sm:p-7">
          ...
          <div className="mt-5 grid gap-4 lg:grid-cols-[.9fr_1.1fr]"><div className="rounded-[26px] border border-white/15 bg-white/[.035] p-4 sm:p-5"><div className="text-[11px] font-black uppercase tracking-[.28em] text-amber-200">You Enter</div><h3 className="mt-2 text-2xl font-black">Information needed from you</h3><div className="mt-4 grid gap-3">
                ...
                {product !== "purchase" && (loanCount > 1
                  ? <LoanAmountRateInput label="Loan 1 / first mortgage" name="mortgage_balance" value={mortgageBalanceInput} onChange={setMortgageBalanceInput} rateName="current_interest_rate_visible" rateValue={currentRateInput} onRateChange={setCurrentRateInput} missingBalance={missingPreviewFields.includes("mortgage_balance")} missingRate={missingPreviewFields.includes("current_interest_rate_visible")} help="Enter the balance and current interest rate for your first mortgage." />
                  : <MoneyInput label="Current mortgage balance" name="mortgage_balance" value={mortgageBalanceInput} onChange={setMortgageBalanceInput} missing={missingPreviewFields.includes("mortgage_balance")} help="Approximate balance remaining on your current mortgage." />
                )}
                {product !== "purchase" && extraLoanInputs.slice(0, visibleExtraLoanCount).map((value, index) => (
                  <LoanAmountRateInput key={index} label={`Loan ${index + 2} / additional property loan`} name={`existing_loan_${index + 2}_balance`} value={value} onChange={(next) => setExtraLoanInputs((prev) => prev.map((item, i) => i === index ? next : item))} rateName={`existing_loan_${index + 2}_interest_rate`} rateValue={extraLoanRateInputs[index] || ""} onRateChange={(next) => setExtraLoanRateInputs((prev) => prev.map((item, i) => i === index ? next : item))} missingBalance={missingPreviewFields.includes(`existing_loan_${index + 2}_balance`)} missingRate={missingPreviewFields.includes(`existing_loan_${index + 2}_interest_rate`)} help="Enter this loan balance and its own interest rate so the payment comparison is more accurate." />
                ))}
                ...
                {product !== "purchase" && <MoneyInput label="Cash amount requested" name="requested_cash" value={requestedCashInput} onChange={setRequestedCashInput} missing={missingPreviewFields.includes("requested_cash")} help="Amount of cash you would like reviewed on top of your existing payoff." />}
                {product === "refinance" && loanCount === 1 && <div className={`premium-input-card ${missingPreviewFields.includes("current_interest_rate_visible") ? "missing-field" : ""}`}><div className="input-header"><span>Client Input</span><b>Current loan interest rate</b></div><div className="money-shell"><input value={currentRateInput} onChange={(e) => { setCurrentRateInput(e.target.value); setMissingPreviewFields((prev) => prev.filter((f) => f !== "current_interest_rate_visible")); }} name="current_interest_rate_visible" className="premium-money-input" placeholder="" inputMode="decimal" /><em>%</em></div><p className="input-help">Used only to estimate what you may be paying now.</p></div>}
...
function MoneyInput({ label, name, value, onChange, help, missing = false }: { label: string; name: string; value: string; onChange: (v: string) => void; help: string; missing?: boolean }) {
  return <div className={`premium-input-card ${missing && !value ? "missing-field" : ""}`}><div className="input-header"><span>Client Input</span><b>{label}</b></div><div className="money-shell"><em>$</em><input name={name} value={value} onChange={(e) => onChange(e.target.value)} className="premium-money-input" placeholder="" inputMode="decimal" /></div><p className="input-help">{missing && !value ? "Required to preview the selected option." : help}</p></div>;
}
function LoanAmountRateInput({ label, name, value, onChange, rateName, rateValue, onRateChange, help, missingBalance = false, missingRate = false }: { label: string; name: string; value: string; onChange: (v: string) => void; rateName: string; rateValue: string; onRateChange: (v: string) => void; help: string; missingBalance?: boolean; missingRate?: boolean }) {
  return <div className={`premium-input-card ${(missingBalance && !value) || (missingRate && !rateValue) ? "missing-field" : ""}`}><div className="input-header"><span>Client Input</span><b>{label}</b></div><div className="loan-duo"><div className={missingBalance && !value ? "missing-subfield" : ""}><div className="duo-label">Balance</div><div className="money-shell"><em>$</em><input name={name} value={value} onChange={(e) => onChange(e.target.value)} className="premium-money-input" placeholder="" inputMode="decimal" /></div></div><div className={missingRate && !rateValue ? "missing-subfield" : ""}><div className="duo-label">Interest Rate</div><div className="money-shell"><input name={rateName} value={rateValue} onChange={(e) => onRateChange(e.target.value)} className="premium-money-input" placeholder="" inputMode="decimal" /><em>%</em></div></div></div><p className="input-help">{(missingBalance && !value) || (missingRate && !rateValue) ? "Required to preview the selected option." : help}</p></div>;
}
