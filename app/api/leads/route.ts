...
export async function POST(req: Request) {
  ...
  if (!supabaseAdmin) {
    return NextResponse.json({ token: t, warning: "Supabase not configured. Demo status page created." });
  }
  ...
  try {
    await fetch(`${process.env.NEXT_PUBLIC_SITE_URL || ""}/api/send-sms`, {
      ...
      body: JSON.stringify({ to: body.phone, name: body.first_name, link: `${process.env.NEXT_PUBLIC_SITE_URL || ""}/status/${t}` })
    });
  } catch {}
