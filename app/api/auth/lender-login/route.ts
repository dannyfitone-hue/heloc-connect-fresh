...
export async function POST(req: Request) {
  try {
    ...
    if (!supabaseAdmin) return NextResponse.redirect(new URL("/lender-login?error=supabase", req.url), 303);
    if (!email || !password) return NextResponse.redirect(new URL("/lender-login?error=missing", req.url), 303);

    const { data: users, error } = await supabaseAdmin.from("lender_users").select("*").eq("email", email).limit(1);
    if (error) {
