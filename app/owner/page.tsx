      ...
      <header className="border-b border-white/10 bg-[#0f0e0a]/95 px-5 py-5">
        <div className="mx-auto flex max-w-[1500px] flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap gap-3 text-sm font-black">
            ...
            <a href="/api/auth/logout" className="rounded-2xl bg-[#d4af37] px-5 py-3 text-[#0b0a07]">Logout</a>
          </div>
        </div>
      </header>
      ...
      <section className="mx-auto max-w-[1500px] px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="rounded-[34px] border border-white/10 bg-gradient-to-br from-[#18140c] to-[#0b0a07] p-6 shadow-2xl">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <div className="text-xs font-black uppercase tracking-[.35em] text-[#d4af37]">Live Funding Platform</div>
            ...
            <div className="rounded-2xl border border-amber-400/30 bg-amber-400/10 px-5 py-4 font-black text-amber-200">
              ● Supabase Connected
            </div>
          </div>

          <div className="mt-8 grid gap-4 md:grid-cols-5">
            <div className="rounded-3xl border border-white/10 bg-[#15120b] p-5"><div className="text-sm font-black text-white/50">Total Leads</div><div className="mt-2 text-4xl font-black">{leads.length}</div></div>
            <div className="rounded-3xl border border-white/10 bg-[#15120b] p-5"><div className="text-sm font-black text-white/50">Network Lenders</div><div className="mt-2 text-4xl font-black text-[#d4af37]">{lenders.length}</div></div>
            <div className="rounded-3xl border border-white/10 bg-[#15120b] p-5"><div className="text-sm font-black text-white/50">Assigned Leads</div><div className="mt-2 text-4xl font-black">{leads.filter(l=>l.assigned_lender_id).length}</div></div>
            <div className="rounded-3xl border border-white/10 bg-[#15120b] p-5"><div className="text-sm font-black text-white/50">Requested Funding</div><div className="mt-2 text-3xl font-black">{money(totalRequested)}</div></div>
            <div className="rounded-3xl border border-white/10 bg-[#15120b] p-5"><div className="text-sm font-black text-white/50">Funded Volume</div><div className="mt-2 text-3xl font-black text-amber-300">{money(funded)}</div></div>
          </div>
        </div>
        ...
        <div className="mt-6 rounded-[34px] border border-amber-300/15 bg-[#11100b] p-5 shadow-2xl sm:p-6">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <div className="text-xs font-black uppercase tracking-[.35em] text-amber-200">Automation Center</div>
            ...
            <div className="rounded-2xl border border-amber-400/30 bg-amber-400/10 px-4 py-3 text-sm font-black text-amber-200">
          ...
          <div className="mt-6 grid gap-4 xl:grid-cols-[.85fr_1.15fr]">
            <div className="rounded-3xl border border-[#d4af37]/25 bg-gradient-to-br from-[#17130b] to-[#11100b] p-5 shadow-xl">
              <div className="text-xs font-black uppercase tracking-[.3em] text-[#d4af37]">Instant SMS</div>
              ...
              <form action="/api/owner/instant-sms" method="post" className="mt-5 grid gap-3">
                <input name="phone" placeholder="Client phone number" className="rounded-2xl border border-white/10 bg-[#0f0e0a] p-4 font-bold text-white outline-none focus:border-[#d4af37]/60" />
                <textarea name="message" rows={5} maxLength={1500} placeholder="Type SMS message here" className="rounded-2xl border border-white/10 bg-[#0f0e0a] p-4 font-semibold leading-relaxed text-white outline-none focus:border-[#d4af37]/60" />
                ...
                <button className="rounded-2xl bg-[#d4af37] p-4 font-black text-[#0b0a07]">Send Instant SMS</button>
              </form>
            </div>
            <div className="rounded-3xl border border-white/10 bg-[#13100a] p-5">
              <div className="text-xs font-black uppercase tracking-[.3em] text-amber-200">SMS Visibility</div>
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-3">
                <div className="rounded-2xl border border-amber-400/20 bg-amber-400/10 p-4"><b className="text-amber-200">Received</b><p className="mt-1 text-xs text-white/50">Logged after system/manual send attempt.</p></div>
        ...
        <div className="mt-6 grid gap-6 xl:grid-cols-[.75fr_1.25fr]">
          <div className="rounded-[34px] border border-white/10 bg-[#11100b] p-6 shadow-2xl">
            ...
            <form action="/api/owner/create-lender" method="post" className="mt-5 grid gap-3">
              <input name="lender_name" required placeholder="Lender / Agent Name" className="rounded-2xl border border-white/10 bg-[#0f0e0a] p-4" />
              <input name="company_name" placeholder="Mortgage Company Name" className="rounded-2xl border border-white/10 bg-[#0f0e0a] p-4" />
              <input name="email" type="email" required placeholder="Login Email" className="rounded-2xl border border-white/10 bg-[#0f0e0a] p-4" />
              <input name="phone" placeholder="Phone" className="rounded-2xl border border-white/10 bg-[#0f0e0a] p-4" />
              <input name="password" required placeholder="Create Password" className="rounded-2xl border border-white/10 bg-[#0f0e0a] p-4" />
              <button className="rounded-2xl bg-[#d4af37] p-4 font-black text-[#0b0a07]">Create Lender User</button>
            </form>

            <h3 className="mt-8 text-xl font-black">Existing Network Lenders</h3>
            <div className="mt-4 grid gap-3">
              {lenders.length ? lenders.map((u) => (
                <div key={u.id} className="rounded-2xl border border-white/10 bg-[#13100a] p-4">
          ...
          <div className="rounded-[34px] border border-white/10 bg-[#11100b] p-4 shadow-2xl sm:p-6">
            <div className="mb-5 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              ...
              <div className="rounded-full border border-[#d4af37]/40 bg-[#d4af37]/10 px-4 py-2 text-xs font-black uppercase tracking-[.2em] text-[#d4af37]">
                Live Lead Flow
              </div>
            </div>
            ...
            {leads.length === 0 ? (
              ...
              <div className="grid gap-4">
                {leads.map((l) => (
                  <div key={l.id} className="rounded-3xl border border-white/10 bg-[#13100a] p-5">
                    <div className="grid gap-5 xl:grid-cols-[1.1fr_.9fr_.95fr]">
                      <div>
                        ...
                        <p className="mt-2 text-xs font-black text-amber-300">Assigned: {l.assigned_agent || (l.assigned_lender_id ? "Assigned" : "Unassigned")}</p>
                        <a className="mt-4 inline-flex rounded-2xl border border-white/10 px-4 py-2 text-sm font-black text-[#d4af37]" href={`/status/${l.token}`} target="_blank">
                      ...
                      <div className="grid gap-3">
                        <form action="/api/owner/assign-lead" method="post" className="rounded-2xl border border-white/10 bg-[#0f0e0a] p-4">
                          <input type="hidden" name="leadId" value={l.id} />
                          <label className="text-xs font-black uppercase tracking-[.2em] text-white/50">Assign To Lender</label>
                          <select name="lenderId" defaultValue={l.assigned_lender_id || ""} className="mt-2 w-full rounded-2xl border border-white/10 bg-[#13100a] p-3 font-bold text-white">
                          ...
                          <button className="mt-3 w-full rounded-2xl bg-amber-400 p-3 font-black text-[#0b0a07]">Assign Lead</button>
                        </form>
                        ...
                        <form action="/api/owner/update-status" method="post" className="rounded-2xl border border-white/10 bg-[#0f0e0a] p-4">
                          ...
                          <textarea name="notes" placeholder="Internal notes / lender notes" defaultValue={l.notes || ""} className="mt-3 h-20 w-full rounded-2xl border border-white/10 bg-[#13100a] p-3 font-semibold text-white" />
                          <button className="mt-3 w-full rounded-2xl bg-gradient-to-b from-[#d4af37] to-[#b88920] p-3 font-black text-[#0b0a07]">Save Update</button>
                        </form>
                        ...
                        <div className="rounded-2xl border border-white/10 bg-[#0f0e0a] p-4">
                          ...
                          <form action="/api/documents/request" method="post" className="mt-3 grid gap-3">
                            ...
                            <button className="w-full rounded-2xl bg-[#d4af37] p-3 font-black text-[#0b0a07]">Request Docs</button>
                          </form>
                        ...
                        <div className="rounded-2xl border border-amber-300/15 bg-[#0f0e0a] p-4">
                          <div className="flex items-center justify-between gap-3">
                            <div>
                              <label className="text-xs font-black uppercase tracking-[.2em] text-amber-200">SMS Timeline</label>
                              <p className="mt-1 text-xs font-bold text-white/45">What this client has received, failed attempts, and manual texts.</p>
                            </div>
                            <span className="rounded-full border border-amber-300/20 bg-amber-300/10 px-3 py-1 text-[10px] font-black uppercase tracking-[.18em] text-amber-100">Live Log</span>
                          </div>
                          ...
                          <form action="/api/owner/instant-sms" method="post" className="mt-4 grid gap-2">
                            ...
                            <input type="hidden" name="returnTo" value="/owner" />
                            <textarea name="message" rows={3} maxLength={1500} placeholder={`Send manual SMS to ${l.phone || "this client"}`} className="w-full rounded-2xl border border-white/10 bg-[#13100a] p-3 text-sm font-semibold text-white outline-none focus:border-amber-300/60" />
                            <button className="rounded-2xl bg-amber-300 p-3 text-sm font-black text-[#0b0a07]">Send Manual SMS To This Lead</button>
                          </form>
                              ...
                              <>
                                {(l.smsLogs || []).slice(0, 4).map((log: any) => (
                                  <div key={`sms-${log.id}`} className="rounded-xl border border-white/10 bg-black/20 p-3 text-xs">
                                    <div className="flex flex-wrap items-center justify-between gap-2">
                                      <b className={String(log.delivery_status || "").toLowerCase().includes("fail") ? "text-red-300" : "text-amber-300"}>{log.delivery_status || "sent"}</b>
                                ...
                                {(l.notes || []).filter((n: any) => String(n.note || "").toLowerCase().includes("sms")).slice(0, 5).map((note: any) => (
                                  <div key={`note-${note.id}`} className="rounded-xl border border-white/10 bg-black/20 p-3 text-xs">
                                    <div className="flex flex-wrap items-center justify-between gap-2">
                                      <b className={String(note.note || "").toLowerCase().includes("not sent") || String(note.note || "").toLowerCase().includes("failed") ? "text-red-300" : "text-amber-300"}>SMS Event</b>
