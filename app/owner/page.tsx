      ...
      <section className="mx-auto max-w-[1500px] px-4 py-8 sm:px-6 lg:px-8">
        <div className="rounded-[34px] border border-white/10 bg-gradient-to-br from-[#0b1b2e] to-[#06111f] p-6 shadow-2xl">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-3xl text-base font-semibold text-white/70">
                Track new homeowner leads, update client status pages, review calculator data, and manage the mortgage company matching process.
              </p>
            </div>
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-4">
            <div className="rounded-3xl border border-white/10 bg-[#08182b] p-5">
              <div className="text-sm font-black text-white/50">Total Leads</div>
              <div className="mt-2 text-4xl font-black">{leads.length}</div>
            </div>
            <div className="rounded-3xl border border-white/10 bg-[#08182b] p-5">
              <div className="text-sm font-black text-white/50">New / Processing</div>
              <div className="mt-2 text-4xl font-black text-[#f6c15a]">{statCount(leads, "Application") + statCount(leads, "Processing")}</div>
            </div>
            <div className="rounded-3xl border border-white/10 bg-[#08182b] p-5">
              <div className="text-sm font-black text-white/50">Requested Funding</div>
              <div className="mt-2 text-3xl font-black">{money(totalRequested)}</div>
            </div>
            <div className="rounded-3xl border border-white/10 bg-[#08182b] p-5">
              <div className="text-sm font-black text-white/50">Funded Volume</div>
              <div className="mt-2 text-3xl font-black text-emerald-300">{money(funded)}</div>
            </div>
          </div>
        </div>

        <div className="mt-6 rounded-[34px] border border-white/10 bg-[#071421] p-4 shadow-2xl sm:p-6">
          <div className="mb-5 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              <h2 className="text-2xl font-black">New Leads & Client Status Control</h2>
              <p className="mt-1 text-sm font-semibold text-white/55">Changing a status here updates the client's private status page.</p>
            </div>
                ...
                <div key={l.id} className="rounded-3xl border border-white/10 bg-[#091a2f] p-5">
                  <div className="grid gap-5 xl:grid-cols-[1.1fr_.9fr_.9fr]">
                    ...
                    <form action="/api/owner/update-status" method="post" className="rounded-2xl border border-white/10 bg-[#06101d] p-4">
                      ...
                      <textarea name="notes" placeholder="Internal notes / lender notes" defaultValue={l.notes || ""} className="mt-3 h-24 w-full rounded-2xl border border-white/10 bg-[#091a2f] p-3 font-semibold text-white" />
                      ...
                      <div className="mt-3 text-xs font-semibold text-white/45">
                        Doc request options: {DOCUMENT_TYPES.slice(0, 4).join(", ")}
                      </div>
                    </form>
                  </div>
                </div>
